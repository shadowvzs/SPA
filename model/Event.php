...
class Event extends Model {
    ...
    $data = [
      ...
      'guests' => static::execQuery("SELECT g.id, u.name, g.title, g.created FROM `guests` as g LEFT JOIN `users` as u on g.user_id = u.id WHERE g.status <= $role AND g.updated >= CURDATE() ORDER BY g.created DESC LIMIT 0, 10"),
			'news' => "<h2>".$newsData['title']."</h2><p>".$newsData['message']."</p><div class='calendarIcon'><img src='./img/icons/menu/event.png'></div>"
		];
