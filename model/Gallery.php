...
class Gallery extends Model {
	...
	protected function index() {
		...
		return $this->sendResponse([$albums, '.album-box-container'], 'build');
	}
	...
	protected function addAlbum($data=null){
		...
		if ($id > 0) {
            ...
            if(!empty(static::getBySlug($newAlbum['slug']))) {
                return static::refuseData('Már létezik album ezen a néven!');
            }
        }

		$saved = $this->save($newAlbum, true);
		if(!empty($saved)) {
			$saved['id'] = $id > 0 ? $id : static::inserted_id();
