...
(function() {
	...
	function Router(middleware) {
		...
		function validateRoute(){
			...
			if (!url_array[0].length) { url_array[0] = "home"; }
			for ( route of routes ) {
	...
	const pages = {
		...
		gallery_index: {
			...
			component: {
				...
				// custom right click menu component for album index page
				contextMenu: {
					condition: {
						role: 2
					},
				...
				// file uploader component settings for album index page
				fileUploader: {
					condition: {
						role: 2			// required role lv
					},
					limit: {
						size: 2048, 	// 2048 kbyte per file, default php upload limit
			...
			component: {
				Guestbook: {
					...
					datasource: {
							...
							activate: MODEL_PATH+'Guestbook.php?action=activate',
	...
	function View () {
		...
		const tFunc = {
			audioPlayer(setup) {
				return `<div id="audioPlayer">
						...
						<div class="details not_selectable">
							...
							<center>
								<div href="*" data-action="component/${setup.name}/button/playButton" class="button">
									<div class="play"></div>
								</div>
								<div href="*" data-action="component/${setup.name}/button/pauseButton" class="button">
									<div class="pause"></div>
								</div>
								<div href="*" data-action="component/${setup.name}/button/stopButton" class="button">
									<div class="stop"></div>
								</div>
							</center>
						</div>
			...
			guestBox(array) {
				let guest, str = '<h1>Vendégek</h1>';
				if (array.length > 0) {
					for (guest of array) {
						str += `<div class="guest"><h2>${guest.title}</h2><br>${guest.message}<p>${guest.created}</p></div>`;
					}
				} else {
			...
			guestbookComment( {id, name, created, message, status, userId} = data ) {
			//	return console.log(data);
				let condition =  (Auth.role > 2 || (userId == Auth.userId) && userId != 0),
					act = (id, type, status) => condition && status == 0 ? `<span class="${type}"><a href="*" data-action="component/Guestbook/activate/${id}">&#10003;</a></span>` : '',
				...
				return `<div class="comment" id="comment_${id}">
							<span class="name">${name}</span> <span class="time">${created}</span>${del(id, 'delete')+edit(id, 'edit', userId)+act(id, 'approve', status)}
							<div class="message">${message}</div>
						</div>`;
			},
			guestbook (data) {
					let str = data.length === 0 ? `<br><center><h2>Nincs bejegyzés</h2></center>` : ``, comment,
		...
		function terminate() {
			let {dom, title, data, component, componentData} = pages.current;
	...
	function Controller(middleware){
		...
		global.pageContent = document.body.querySelector('.content.page');
			...
			if ( eventData && (dateArr || dateArr === 0)) {
				if (typeof dateArr === "object") {
					...
					if (stack) {
						...
						if (count > 0) { div.dataset.counter = count; }
					}
				}
			}
	...
	function GuestbookComponent (setup, req) {
		const componentKey = setup.name,
					...
					data => {
						if (data.modelData.deleted) {
							const component = pages.current.componentData[componentKey],
								...
								dom = document.getElementById(setup.form.prefix+id);
							if (dom) { dom.remove(); }
							for (let key of keys) {
								if (component[key].id == id) {
									delete component[key];
									return notify.add('Hozzászólás törölve', 'success');
								}
							}
						}
					},
				...
				for (comment of comments) {
					...
					if (!dom) {
						if (comment.status == 1) {
							...
							notify.add('A hozzászólás elmentve, de jóvá kell hagyni egy moderátornak!', 'success')
						}
					} else {
			...
			function newComment() {
				const isLogged = Auth.userId > 0;
				setContent({
					name: [isLogged ? 'session' : '', isLogged ? 'none' : 'block'],
					email: [isLogged ? 'session@session.com' : '', isLogged ? 'none' : 'block'],
					id: ['0', 'block'],
					message: ['', 'block'],
				});
			}
			...
			function activate(id) {
				req.post(
					setup.datasource.activate,
					{ param: {id:id} },
					data => {
						if (data.modelData.status) {
							const component = pages.current.componentData[componentKey],
								keys = Object.keys(component),
								dom = document.getElementById(setup.form.prefix+id);
							if (dom) {
								let activateDOM = dom.querySelector('.approve');
								if (activateDOM) { activateDOM.remove(); }
							}
							for (let key of keys) {
								if (component[key].id == id) {
									component[key].status = 1;
									return notify.add('Hozzászólás elfogadva', 'success');
								}
							}
						}
					},
		...
		function AlbumManager (setup, req) {
			...
			renderFunc = {
				add(data) {
					...
					opt.value = data.id;
					selAlbum.add(opt);
					thumbContainer.insertAdjacentHTML('afterbegin', view.getContent('albumCreate', data));
