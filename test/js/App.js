...
console.log('ajax data', data, url);
	...
	/*
			...
			//error_index(data) {
			//	console.log(this);
				//return tFunc.pageContainer(data.controller, data.action, tFunc.errorMsg(data.param));
			//}			
		};
		*/
...
*/		
	...
	var View = function(){
		...
		var tFunc = {
			...
			pageContainer(cntrllr, ctn, tmplt) { return `<div class="${cntrllr}"><div class="${ctn}" style="animation: fadeOut 0.5s;">${tmplt}</div></div>`; },
		...
		function terminate(){
			let {dom, bone, title, data, component} = pages.current;
			...
			// remove the page, still not exist page depended eventListeners but if will, then need remove here
			dom.innerHTML = "";
			// total clean
			pages.current = {}
	...
	function Calendar(settings={range:{}}) {
		...
		yearStack = [], yearStackIndex, output, callback, eventData, output, selEvents, dateKey, calToggleButtonEvent,
